...
import { SLOTS_CONFIG_V3 as SLOTS_CONFIG } from '../rtpConfig-v3'
...
export const getSlotCombination = (
  ...
  const getSymbolByName = (symbolName: string): SlotItem => {
    ...
    const symbolFromConfig = SLOTS_CONFIG.symbols.find((s: { name: string, multiplier: number }) => s.name === symbolName)
    if (!symbolFromConfig) {
