...
export default function GameLobby({ onJoinSingleplayer, onJoinMultiplayer, onCreateSingleplayer, onCreateMultiplayer }: GameLobbyProps) {
  ...
  const availableMultiplayerGames = games
    .filter((game: any) => game.players.length < POKER_SHOWDOWN_CONFIG.MAX_PLAYERS && game.state.waiting)
  ...
  const renderSingleplayerTables = () => (
    <LobbyContainer>
      <TablesSection>
                    <SectionTitle>🎲 Active Poker Tables</SectionTitle>
        <TablesGrid>
          {SINGLEPLAYER_TABLES.map(table => (
            ...
            >
              ...
              <JoinButton onClick={onJoinSingleplayer}>
                🎯 Join Table
              </JoinButton>
            </TableCard>
          ))}
  ...
  return (
    <>
      <GambaUi.Portal target="stats">
        <Header>
          <Title>� Poker Showdown Lobby</Title>
          <Subtitle>
            {viewMode === 'info' 
