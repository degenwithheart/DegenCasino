...
const PageWrapper = styled.div<{ $colorScheme?: any }>`
  ...
  &::before {
    ...
    bottom: 0;
    background:
      radial-gradient(circle at 25% 25%, ${({ $colorScheme }) => $colorScheme?.colors?.primary || 'rgba(255, 215, 0, 0.03)'} 0%, transparent 50%),
      radial-gradient(circle at 75% 75%, ${({ $colorScheme }) => $colorScheme?.colors?.secondary || 'rgba(162, 89, 255, 0.03)'} 0%, transparent 50%);
    pointer-events: none;
    z-index: 0;
  }
`;
...
const TokenSelectContainer = styled.div<{ $colorScheme?: any }>`
  background: ${({ $colorScheme }) => $colorScheme?.colors?.surface || 'rgba(24, 24, 24, 0.9)'};
  backdrop-filter: blur(20px);
  border-radius: 16px;
...
export default function SelectTokenPage() {
  ...
  return (
    <PageWrapper $colorScheme={colorScheme}>
      ...
      <ContentWrapper>
        <TokenSelectContainer $colorScheme={colorScheme}>
