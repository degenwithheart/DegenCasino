...
export const BET_ARRAYS_V3 = {
  ...
  'mines': {
    ...
    calculateBetArray: (mineCount: number, cellsRevealed: number) => {
      ...
      // Build exact event probabilities: probability that player reveals exactly k safe cells
      const probs: number[] = []
      for (let k = 0; k <= GRID_SIZE; k++) {
        if (k < safeCells) {
          // probability that first k picks are safe and next pick is a mine
          let p = 1
          for (let i = 0; i < k; i++) {
            p *= (safeCells - i) / (GRID_SIZE - i)
          }
          // next pick is a mine
          if (GRID_SIZE - k > 0) {
            p *= (mineCount) / (GRID_SIZE - k)
          } else {
            p = 0
          }
          probs.push(p)
        } else if (k === safeCells) {
          // probability of revealing all safe cells (no mine left)
          let p = 1
      ...
      const target = RTP_TARGETS_V3['mines']

      // Count of non-zero probability outcomes
      const nonZeroCount = probs.filter(p => p > 0).length || 1

      // Set multipliers proportional to 1/p_i and normalized so sum(p_i * m_i) = target
      // m_i = C * (1 / p_i), C = target / nonZeroCount
      const C = target / nonZeroCount
      for (let i = 0; i < probs.length; i++) {
        const p = probs[i]
        if (p > 0) {
          const m = C * (1 / p)
          betArray[i] = parseFloat(m.toFixed(4))
        } else {
          betArray[i] = 0
        }
      }
