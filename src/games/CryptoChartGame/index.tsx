...
export default function CryptoChartGame() {
  ...
  const animate = (current: number, target: number, win: boolean, result: { payout: number; wager: number }) => {
    ...
    if (current >= target) {
      ...
      }, 1000) // Delay to let the status display show before overlay
      return
    }
  ...
  const play = async () => {
    ...
    try {
      ...
      if (win) {
        // Sometimes show a much higher multiplier for dramatic effect
        const extraMoon = Math.random() < 0.5
        if (extraMoon) {
          // 1x to 3x above pick
          targetMultiplier = multiplierTarget + Math.random() * multiplierTarget * 2
        } else {
          targetMultiplier = multiplierTarget
        }
      } else {
        // Sometimes crash much earlier for dramatic effect
        const earlyRug = Math.random() < 0.5
        if (earlyRug) {
          // Crash anywhere between 1x and (pick - 0.5)
          targetMultiplier = 1 + Math.random() * Math.max(0.5, multiplierTarget - 1.5)
        } else {
          targetMultiplier = calculateBiasedLowMultiplier(multiplierTarget)
        }
      }
      const finalResult = { payout: result.payout, wager }
      animate(0, targetMultiplier, win, finalResult)
    } catch {
      setGameState('idle')
      setLastPayout(null)
    }
  ...
  const priceColor = simulatedPrice > prevPrice
    ? '#00ff55'
    : simulatedPrice < prevPrice
      ? '#ff4444'
      : '#ffffff'

  // Price range for chart and sparkline calculations
  const minPrice = 180 // Solana price baseline
  const maxPrice = 1000000000 // 1 billion

  // Clamp last candle close within [0, 1000]
  const lastCandleClose = candles.length > 0 ? candles[candles.length - 1].close : minPrice
  const clampedCandleClose = Math.min(maxPrice, Math.max(minPrice, lastCandleClose))
  // Sparkline progress from minPrice..maxPrice range
  const sparkProgress = Math.max(0, Math.min(1, (clampedCandleClose - minPrice) / (maxPrice - minPrice)))
  ...
  // isPlaying is true if the game is animating or not idle
  const isPlaying = gamba.isPlaying || gameState !== 'idle' || animationRef.current !== undefined

  // Map price (0..1000) to Y position (invert y-axis)
  // Map price (minPrice..maxPrice) to Y position (invert y-axis)
  ...
  // Generate sparkline points for candles, clamp close to [0, 1000]
  // Generate sparkline points for candles, clamp close to [minPrice, maxPrice]
      ...
      <GambaUi.Portal target="screen">
        <GameScreenLayout
                ...
                <ChartWrapper style={{ display: 'flex', width: '100%', height: '100%' }}>
                  ...
                  <div style={{ flex: 1, position: 'relative' }}>
                    <svg
                      width="100%"
                      height="100%"
                      viewBox={`${offsetX} 0 ${viewBoxWidth + 40} ${viewBoxHeight}`}
                      preserveAspectRatio="none"
                    >
                      {[...Array(5)].map((_, i) => {
                        const y = (i + 1) * (viewBoxHeight / 6)
                        // Price labels from top to bottom - fixed 1000, 750, 500, 250, 0
                      ...
                      <line
                        x1={offsetX}
                        x2={offsetX + viewBoxWidth}
                        y1={priceToY(simulatedPrice)}
                        y2={priceToY(simulatedPrice)}
                        stroke="#00ff55"
                        strokeWidth="2"
                        strokeDasharray="4 2"
                      />
                      {/* Sparkline "heartbeat" polyline with glow and animated dashoffset */}
                      <defs>
                      ...
                      <polyline
                        ...
                        strokeDasharray="8 4"
                        style={{
                          transition: 'points 0.1s ease-in-out',
                          animation: 'dashmove 1.2s linear infinite',
                        }}
                      />
                      {/* Fading trail behind sparkline (EKG effect) */}
                ...
                >
                  ...
                  {gameState === 'win' && lastPayout !== null && (
                    <div style={{ fontSize: 20, color: '#ffd700', fontWeight: 'bold' }}>
                      Total Won:{' '}
                      {`${(lastPayout / Math.pow(10, token?.decimals || 6)).toLocaleString(undefined, {
                        maximumFractionDigits: token?.decimals || 2,
          ...
          right={
            ...
            >
              <div style={{ color: '#ccc', fontSize: 14 }}>Price Info</div>
              <div
                style={{
                  fontSize: 20,
                  fontWeight: 'bold',
              ...
              >
                ${simulatedPrice.toLocaleString(undefined, { maximumFractionDigits: 5 })}
              </div>
              <div style={{ marginTop: 12 }}>
                <div style={{ color: '#888', fontSize: 12 }}>Range:</div>
                <div style={{ color: '#fff', fontWeight: 'bold', fontSize: 14 }}>
                  0.00 - 1,000,000,000.00
                </div>
              </div>
              <svg width="100%" height="24" viewBox={`0 0 160 24`} style={{ marginTop: 10 }}>
                <defs>
                  <linearGradient id="panelSparklineGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#00ff55" />
                    <stop offset="100%" stopColor="#007700" />
                  </linearGradient>
                  <filter id="panelGlow" x="-50%" y="-50%" width="200%" height="200%">
                    <feDropShadow dx="0" dy="0" stdDeviation="2" floodColor="#00ff55" floodOpacity="0.5" />
                  </filter>
                </defs>
                <polyline
                  points={candles.slice(-24).map((c, i) => `${i * 7},${22 - ((Math.min(maxPrice, Math.max(minPrice, c.close)) - minPrice) / (maxPrice - minPrice)) * 22}`).join(' ')}
                  fill="none"
                  stroke="url(#panelSparklineGradient)"
                  strokeWidth={2}
              ...
              <div style={{ marginTop: 14 }}>
                <div style={{ fontSize: 12, color: '#aaa' }}>Multiplier</div>
                <div style={{ fontWeight: 'bold', fontSize: 16, color: '#fff', transition: 'transform 0.3s' }}>
