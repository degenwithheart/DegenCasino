      ...
      {selectedCount > 0 && (
        <div style={{ marginBottom: '16px' }}>
          <h4 style={{ 
            ...
            Potential Payouts ({selectedCount} selected)
          </h4>
          <div style={{ 
            ...
            {Object.entries(payoutTable).map(([hits, multiplier]) => (
              <div key={hits} style={{ 
                display: 'flex', 
                ...
                <span style={{ color: '#ffd700' }}>
                  {multiplier}x ({formatAmount(wager * multiplier)} {token?.symbol})
                </span>
              </div>
            ))}
          </div>
      ...
      {currentResult && (
        <div style={{ 
          ...
          <div style={{ fontSize: '12px', color: '#fff', marginBottom: '4px' }}>
            Selected: {currentResult.selectedNumbers.join(', ')}
          </div>
          <div style={{ fontSize: '12px', color: '#fff', marginBottom: '4px' }}>
            Drawn: {currentResult.drawnNumbers.join(', ')}
          </div>
          <div style={{ fontSize: '12px', color: '#fff' }}>
            <strong>{currentResult.hits} hits</strong> • {currentResult.multiplier.toFixed(1)}x
          </div>
        </div>
      )}
