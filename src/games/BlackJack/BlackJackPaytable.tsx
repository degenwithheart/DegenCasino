    ...
    <div style={{
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <div style={{ 
          ...
          {Object.entries(payoutStructure).map(([outcome, payout]) => (
      ...
      {currentResult && (
        <div style={{ 
          marginBottom: '16px',
          padding: '12px',
          background: currentResult.outcome === 'blackjack' 
            ? 'rgba(255, 215, 0, 0.15)' 
            : currentResult.wasWin 
            ? 'rgba(34, 197, 94, 0.15)' 
            : currentResult.outcome === 'push'
            ? 'rgba(139, 69, 19, 0.15)'
            : 'rgba(239, 68, 68, 0.15)',
          borderRadius: '8px',
          border: `1px solid ${
            currentResult.outcome === 'blackjack' ? '#ffd700' :
            currentResult.wasWin ? '#22c55e' : 
            currentResult.outcome === 'push' ? '#8b4513' : '#ef4444'
          }33`
        }}>
          <div style={{ 
            fontSize: '13px', 
            fontWeight: '600', 
            marginBottom: '6px',
            color: currentResult.outcome === 'blackjack' ? '#ffd700' :
                   currentResult.wasWin ? '#22c55e' : 
                   currentResult.outcome === 'push' ? '#8b4513' : '#ef4444'
          }}>
            Latest Hand - {currentResult.outcome.toUpperCase()}
          </div>
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <div style={{ 
          ...
          <div style={{ color: '#888' }}>Avg Player:</div>
          <div style={{ color: '#fff', textAlign: 'right' }}>
            {sessionStats.averagePlayerTotal.toFixed(1)}
          </div>
          
          <div style={{ color: '#888' }}>Avg Dealer:</div>
          <div style={{ color: '#fff', textAlign: 'right' }}>
            {sessionStats.averageDealerTotal.toFixed(1)}
          </div>
          
          <div style={{ color: '#888' }}>Player Busts:</div>
          <div style={{ color: '#ef4444', textAlign: 'right' }}>{sessionStats.bustCount}</div>
          
          <div style={{ color: '#888' }}>Dealer Busts:</div>
          <div style={{ color: '#22c55e', textAlign: 'right' }}>{sessionStats.dealerBustCount}</div>
        </div>
      </div>
            ...
            {results.map((result, index) => (
              <div 
                ...
                style={{ 
                  padding: '8px',
                  marginBottom: '6px',
                  background: result.outcome === 'blackjack' 
                    ? 'rgba(255, 215, 0, 0.1)' 
                    : result.wasWin 
                    ? 'rgba(34, 197, 94, 0.1)' 
                    : result.outcome === 'push'
                  ...
                  fontSize: '11px',
                  border: `1px solid ${
                    result.outcome === 'blackjack' ? '#ffd700' :
                    result.wasWin ? '#22c55e' : 
                    result.outcome === 'push' ? '#8b4513' : '#ef4444'
                  }22`
                }}
              >
                <div style={{ 
                  ...
                  <span style={{ 
                    color: result.outcome === 'blackjack' ? '#ffd700' :
                           result.wasWin ? '#22c55e' : 
                           result.outcome === 'push' ? '#8b4513' : '#ef4444',
                    fontWeight: '600'
                  }}>
                  ...
                  <span style={{ color: '#888' }}>
                    {result.playerTotal} vs {result.dealerTotal}
                  </span>
                </div>
                <div style={{ color: '#ccc' }}>
