...
import '../../types/styled'; // Import styled-components type definitions
import { useGameStats } from '../../hooks/game/useGameStats';
import { makeDeterministicRng } from '../../fairness/deterministicRng';
import GameplayFrame, { GameplayEffectsRef } from '../../components/Game/GameplayFrame';
import { useGameMeta } from '../useGameMeta';
import { BET_ARRAYS_V2 } from './config';

// RTP targets for games (house edge = 1 - RTP)
        ...
        <ResultDisplay $hasResult={hasPlayed && lastGameResult !== null} $isWin={lastGameResult === 'win'}>
          <ResultText $hasResult={hasPlayed && lastGameResult !== null} $isWin={lastGameResult === 'win'}>
            {hasPlayed && lastGameResult !== null ?
              `${lastGameResult === 'win' ? 'WIN!' : 'LOSE'} - ${coinResults.filter((r: 'heads' | 'tails' | null, i: number) => i < numCoins && r === side).length}/${numCoins} ${side}` :
