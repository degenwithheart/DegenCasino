...
const pickFromRNG = <T>(arr: T[], rng: () => number) => arr[Math.floor(rng() * arr.length)]
...
export const generateBetArray = (
  maxPayout: number,
  wager: number,
  maxLength = 50, // Match Slots111 default maxLength
) => {
  // Return the predefined bet array from config - this ensures fair and auditable odds
  // that match the intended RTP and house edge settings
  return SLOTS_CONFIG.betArray.slice(0, maxLength);
}
...
export const getSlotCombination = (
  ...
  // Helper to get deterministic position in column sequence based on RNG
  const getSymbolFromColumn = (columnIndex: number, rng: () => number): SlotItem => {
    const sequence = COLUMN_SEQUENCES[columnIndex]
    if (!sequence) return SLOT_ITEMS[0] // fallback
    const index = Math.floor(rng() * sequence.length)
    return getSymbolByName(sequence[index])
  }
  ...
  const getUniqueSymbolsForColumn = (columnIndex: number, count: number, rng: () => number): SlotItem[] => {
    ...
    // Create deterministic sequence based on RNG
const shuffled = [...sequence].map((_, i) => {
