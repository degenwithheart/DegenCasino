...
import { LiveGameEcosystem } from './components/LiveGameEcosystem';

interface TraditionalGameCardProps {
    game: GameBundle;
    onClick?: () => void;
...
function TraditionalGameCard({ game, onClick, statsVariant = 'default', playedStats }: TraditionalGameCardProps) {
    ...
    // 3D effect configuration (simplified)
    const CONFIG = {
        baseMaxRotate: 8,
        pressedMaxRotate: 14,
        depthZMultiplier: 22,
    ...
    // Simplified 3D interaction handlers
...
export function GameCard3D({ game, onClick, statsVariant = 'default', playedStats }: GameCard3DProps) {
    ...
    return (
        <>
            {FEATURE_FLAGS.ENABLE_LIVE_GAME_ECOSYSTEMS ? (
                <LiveGameEcosystem game={game} onClick={handleClick} />
            ) : (
                <TraditionalGameCard
                    game={game}
                    onClick={handleClick}
