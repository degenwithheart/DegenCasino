...
// Portal viewport height classes for content that needs to scale with portal container
// Usage: Use --pvh instead of vh for height calculations within portals
// Example: height: calc(80 * var(--pvh)) instead of height: 80vh

// Portal content that should scale with portal height
export const PortalViewportContent = css`
  /* Use portal viewport units for height-based properties */
  max-height: var(--portal-height, 100vh);
  
  /* Example usage patterns: */
  /* height: calc(80 * var(--pvh, 1vh)); // Instead of height: 80vh */
  /* top: calc(10 * var(--pvh, 1vh)); // Instead of top: 10vh */
  /* margin-top: calc(5 * var(--pvh, 1vh)); // Instead of margin-top: 5vh */
  
  /* Width properties remain unchanged */
  width: 100%;
  max-width: 100%;
`;

// Apply portal viewport scaling to common portal content
export const PortalScaledModal = styled.div`
  ${PortalViewportContent}
  
  /* Example modal that scales with portal height */
  position: absolute;
  left: 50%;
  top: 50%;
  ...
  width: 90%;
  max-width: 500px;
  height: calc(80 * var(--pvh, 1vh)); /* Scales with portal height */
  max-height: calc(var(--portal-height, 100vh) - 40px); /* Always fits with padding */
...
// Portal overlay that fills the portal container exactly
export const PortalOverlay = styled.div`
  ...
  /* Content inside can use portal viewport units */
  > * {
    max-height: var(--portal-height, 100vh);
  }
`;
...
/* ========================================
   PORTAL GAME RESPONSIVE SYSTEM
   ======================================== */

// Container query-based responsive portal target
// This allows game content to be responsive to the portal container size
export const ScalingPortalTarget = styled.div`
  position: relative;
  width: 100%;
  height: 100%;
  overflow: hidden;
  ...
  /* Apply responsive styles based on container height */
  @container game-screen (max-height: 400px) {
    font-size: 14px;
    
    /* Scale down game elements for smaller heights */
    --game-scale: 0.8;
    --game-padding: 0.5rem;
    --game-font-size: 12px;
  }
  
  @container game-screen (max-height: 300px) {
    font-size: 12px;
    
    --game-scale: 0.6;
    --game-padding: 0.25rem;
    --game-font-size: 10px;
  }
  
  @container game-screen (min-height: 500px) {
    font-size: 16px;
  ...
  /* Fallback: If container queries aren't supported, use CSS transform scaling */
