...
export const Container = styled.div<{ $colorScheme?: any }>`
  position: relative;
  display: flex;
  flex-direction: column;
  gap: 12px;
  padding: 16px;
  background: ${({ $colorScheme }) => $colorScheme?.colors?.surface || 'linear-gradient(135deg, rgba(15, 18, 27, 0.95) 0%, rgba(24, 24, 24, 0.95) 100%)'};
  border-radius: 20px;
  border: 1px solid ${({ $colorScheme }) => `${$colorScheme?.colors?.primary || '#ffd700'}20`};
  backdrop-filter: blur(20px);
  box-shadow: ${({ $colorScheme }) => $colorScheme?.effects?.glow || '0 8px 32px rgba(0, 0, 0, 0.3), 0 0 0 1px rgba(255, 255, 255, 0.05), inset 0 1px 0 rgba(255, 255, 255, 0.1)'};
`
