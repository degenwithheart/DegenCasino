...
class GlobalTrustAuditor {
  constructor() {
    this.games = [
      ...
      'HiLo', 'BlackJack', 'ProgressivePoker', 'Roulette', 'BlockStack',
  ...
  async extractRtpConfigData() {
    ...
    return {
      ...
      roulette: this.extractRouletteConfig(content)
    };
  }
  ...
  extractRouletteConfig(content) {
    // Roulette uses calculateBetArray function - dynamic values based on bet type
    // Extract common bet type multipliers from BET_TYPES
    const match = content.match(/BET_TYPES:\s*{[\s\S]*?straight:\s*{\s*payout:\s*([\d.]+)[\s\S]*?}/);
    if (match) {
      const straightPayout = parseFloat(match[1]);
      const rtpMultiplier = straightPayout * 0.973; // Apply 97.3% RTP
      return { multipliers: [rtpMultiplier, 1.946, 2.919, 0], isDynamic: true }; // Common roulette payouts
    }
    return { multipliers: [35.054, 17.027, 11.351, 8.784, 5.405, 1.946, 0], isDynamic: true };
  }
  ...
  findTrustIssues(gameName, rtpData, hardcodedChecks, content) {
    ...
    // Check if game uses modern Gamba result pattern
    const usesGambaResult = content.includes('result.payout') || content.includes('result.multiplier');
    const usesResultPayout = content.includes('result.payout');
    const usesResultMultiplier = content.includes('result.multiplier');
    
    // Modern pattern: Games that use result.payout/result.multiplier don't need hardcoded values
    if (usesGambaResult) {
      console.log(`✅ MODERN GAMBA PATTERN: Uses result.payout: ${usesResultPayout}, result.multiplier: ${usesResultMultiplier}`);
      
      // Only flag hardcoded values if they're problematic (not matching outcomes)
      hardcodedChecks.forEach(hardcoded => {
        // Allow common values like 0, 1 for lose/push states
        if (hardcoded !== 0 && hardcoded !== 1) {
          // Check if this hardcoded value exists in rtpConfig outcomes
          if (rtpData.multipliers.length > 0 && !rtpData.multipliers.includes(hardcoded)) {
            issues.push({
              type: 'potentially_problematic_hardcoded',
              hardcoded,
              rtpValues: rtpData.multipliers,
              description: `Uses hardcoded ${hardcoded}x but rtpConfig has [${rtpData.multipliers.join(', ')}] - verify this is intentional`
            });
          }
        }
      });
      
      return issues; // Modern games using result.payout/multiplier are generally trustworthy
    }

    // Legacy pattern check: Games not using result.payout/multiplier
    console.log(`⚠️ LEGACY PATTERN: No result.payout/multiplier usage detected`);
    ...
    // For static games not using modern pattern, check if hardcoded values match rtpConfig
    hardcodedChecks.forEach(hardcoded => {
    ...
    // Check for missing visual logic for rtpConfig values (only for legacy games)
